...
BEGIN
  ...
  IF has_pgss THEN
    ...
    LOOP
      ...
      -- Try a real EXPLAIN (ANALYZE, BUFFERS) inside a subtransaction
      -- with a tight statement timeout so we never run away.
      BEGIN
        PERFORM set_config('statement_timeout', '2000', true);
        EXECUTE 'EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT) ' || q
          INTO plan_text;
      EXCEPTION WHEN OTHERS THEN
        -- Most likely cause: unbound $N parameters from PostgREST-normalized SQL.
        -- Fall back to the parameter-tolerant GENERIC_PLAN.
        plan_mode := 'GENERIC_PLAN (ANALYZE failed: ' || SQLERRM || ')';
        BEGIN
          EXECUTE 'EXPLAIN (GENERIC_PLAN, BUFFERS, FORMAT TEXT) ' || q
            INTO plan_text;
        EXCEPTION WHEN OTHERS THEN
