...
BEGIN
  ...
  IF has_pgss THEN
    ...
    FROM (
      SELECT jsonb_build_object(
        ...
        'over_mean', mean_exec_time > _mean_ms,
        'over_max', max_exec_time > _max_ms
      ) AS row
    ...
    LOOP
      ...
      enriched_failures := enriched_failures || jsonb_build_array(
        fail_elem
          || jsonb_build_object('explain_mode', plan_mode, 'explain', COALESCE(plan_text, 'no plan'))
          - 'query'
      );
    END LOOP;
  ELSE
  ...
  RETURN jsonb_build_object(
    'checked_at', now(),
    'thresholds', jsonb_build_object('mean_ms', _mean_ms, 'max_ms', _max_ms),
...
REVOKE ALL ON FUNCTION public.db_perf_smoke(numeric, numeric) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.db_perf_smoke(numeric, numeric) TO service_role;

-- Drop the older parameterless overload so the API surface is unambiguous.
DROP FUNCTION IF EXISTS public.db_perf_smoke();
