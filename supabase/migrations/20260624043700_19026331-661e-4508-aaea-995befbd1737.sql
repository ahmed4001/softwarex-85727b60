...
CREATE OR REPLACE FUNCTION public.db_perf_smoke(
  _mean_ms numeric DEFAULT 200,
  _max_ms numeric DEFAULT 800,
  _queries jsonb DEFAULT '[]'::jsonb,
  _mean_rows numeric DEFAULT NULL,
  _max_rows numeric DEFAULT NULL
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  eff_mean numeric;
  eff_max numeric;
  eff_mean_rows numeric;
  eff_max_rows numeric;
  over_mean boolean;
  over_max boolean;
  over_mean_rows boolean;
  over_max_rows boolean;
BEGIN
  ...
  IF has_pgss THEN
    ...
    FROM (
      SELECT jsonb_build_object(
        ...
        'rows', rows,
        'mean_rows', CASE WHEN calls > 0 THEN round((rows::numeric / calls), 2) ELSE 0 END,
        'max_rows', rows
      ) AS row
    ...
    LOOP
      ...
      eff_mean := _mean_ms;
      eff_max := _max_ms;
      eff_mean_rows := _mean_rows;
      eff_max_rows := _max_rows;
      matched_rule := NULL;
      ...
      IF jsonb_typeof(_queries) = 'array' THEN
        ...
        LOOP
          IF rule ? 'match' AND position(lower(rule->>'match') in lower(q)) > 0 THEN
            ...
            END IF;
            IF rule ? 'mean_rows' AND jsonb_typeof(rule->'mean_rows') = 'number' THEN
              eff_mean_rows := (rule->>'mean_rows')::numeric;
            END IF;
            IF rule ? 'max_rows' AND jsonb_typeof(rule->'max_rows') = 'number' THEN
              eff_max_rows := (rule->>'max_rows')::numeric;
            END IF;
            EXIT;
          END IF;
        END LOOP;
      ...
      -- Row checks only apply when a threshold is configured (NULL = skip).
      over_mean_rows := eff_mean_rows IS NOT NULL
        AND (fail_elem->>'mean_rows')::numeric > eff_mean_rows;
      over_max_rows := eff_max_rows IS NOT NULL
        AND (fail_elem->>'max_rows')::numeric > eff_max_rows;

      IF NOT (over_mean OR over_max OR over_mean_rows OR over_max_rows) THEN
        CONTINUE;
      END IF;
      ...
      enriched_failures := enriched_failures || jsonb_build_array(
        (fail_elem - 'query')
          || jsonb_build_object(
            ...
            'applied_mean_rows', eff_mean_rows,
            'applied_max_rows', eff_max_rows,
            'over_mean', over_mean,
            'over_max', over_max,
            'over_mean_rows', over_mean_rows,
            'over_max_rows', over_max_rows,
